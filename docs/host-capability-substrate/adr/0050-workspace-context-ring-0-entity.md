...
- **Cross-context binding posture inherited**: WorkspaceContext is a typed identity, not a context-bound observation. Charter inv. 19 (boundary claims execution-context-bound) does not apply at the entity level (workspace identity is execution-context-independent). Other entities that BIND to a WorkspaceContext (per ADR 0037 future amendment for AgentClient × WorkspaceContext) will carry their own execution-context binding.
...
- **Schema-level cross-context refinement** — workspace identity is execution-context-independent; the Decision-style cross-context binding rule does not apply to WorkspaceContext at the entity level. No refinement is needed at Ring 0; the existing §Cross-context enforcement layer rules apply to Decisions / Evidence / CoordinationFact records that REFERENCE WorkspaceContext, not to WorkspaceContext itself.
