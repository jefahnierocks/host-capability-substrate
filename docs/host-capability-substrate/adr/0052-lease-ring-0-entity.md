...
`proposed` (v1 — preemptively absorbs the ADR 0051 v1→v4 cycle lessons: envelope-level superRefine for chain-walk rejection, canonical-concatenation length-prefix discipline, no entity-level `tier_scope` column (ground forbidden-tier defense on `OperationShape.operation_class` source-enum closure), `operation_class_scope` documentation column, lower_snake_case enum compliance, additive D-037-style producer-disjointness rule, walk-depth budget ≤ 64 + cycle-rejection, force-break-grant_kind extension deferred to coordinated `kernel_dashboard` producer ADR change-set)
...
- **Authority-discipline posture (envelope-level kernel-set with field-level exceptions per ADR 0031 v1)**:
  - **Kernel-set** (12 fields): `schema_version`, `lease_id`, `held_by_session_id`, `held_by_agent_client_id`, `acquired_by`, `acquired_at`, `released_at`, `execution_context_id`, `lease_state`, `force_break_grant_id`, `audit_chain_link_hash`, `evidence_refs`
  - **Producer-asserted-kernel-verifiable** (3 fields): `lease_kind`, `scope` (worktree branch's `repository_id` + `workspace_context_id` + `worktree_path`), `valid_until`

  Total: 14 fields ((counted excluding the `scope` union as a single field; the worktree branch contains 3 sub-fields counted separately under producer-asserted-kernel-verifiable for granular authority declaration). Layer 1 mint API verifies producer-asserted values per ADR 0031 v1 §Authority discipline; rejection emits `Decision.reason_kind: 'lease_producer_assertion_unverifiable'` (NEW reservation).
...
- **NEW §Procedure for adding a new lease_kind value rule** registered in `ontology-registry.md` mirroring §Procedure rule patterns from ADR 0048 + ADR 0049 + ADR 0051 v4. Future schema PRs extending the enum:
  ...
  5. **Commit per-`lease_kind` sandbox-acquire rule** (whether sandbox-execution sessions can acquire this kind)
...
- Schema source for related entities:
  ...
  - `packages/schemas/src/entities/evidence.ts` — `Evidence.subject_kind: 'lease'` is registry-canonical reserved (per ADR 0019 v3 + workflow-sequencing investigation §Step 3 batched M1 entities); Zod-defined value lands at downstream Evidence schema PR or as part of this ADR's schema PR change-set
