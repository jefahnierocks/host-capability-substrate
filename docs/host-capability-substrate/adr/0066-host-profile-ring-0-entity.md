...
`hcs-security-reviewer` is load-bearing: `host_identity` is a machine
fingerprint, so the never-the-raw-identifier posture (privacy; charter
sensitive-data discipline) is a central review axis.
...
HostProfile (non-minted Ring 0; structural peer of Capability/SecretReference)
  ...
  os_version         string (min 1; no secret/URI/path shape)
  arch               enum: arm64 | x86_64 | unknown
  host_identity      { kind:   platform_uuid_sha256 | install_id_sha256 | unknown,
                       digest: sha256DigestSchema }   // a NON-REVERSIBLE digest;
                                                      // NEVER a raw serial / UUID
  source_provenance  { authority: 'host_profile_observation', observed_at }
```
...
raw identifier cannot land in the field. Ring 0 validates the digest SHAPE only;
that the digest was actually computed over the named stable identifier (and not,
say, over a value the producer should not have read) is a Ring 1 producer
obligation.

### `source_provenance` and host-authoritative observation

`source_provenance` is a `.strict()` sub-object (`authority` literal
`host_profile_observation`, disjoint from `evidenceAuthoritySchema` and
conferring no authority by itself; `observed_at`), mirroring the
`Capability` / `SecretReference` non-minted provenance pattern. Two charter
...
- The host fingerprint is stored as a non-reversible digest; a raw machine
  identifier cannot land in the entity (structural `sha256:` enforcement).
...
1. Add `packages/schemas/src/entities/host-profile.ts`:
   ...
   `os_name`, `arch`, and `host_identity.kind` enums; the
   `hostProfileHostIdentitySchema` `.strict()` sub-object (`kind` +
   `sha256DigestSchema` `digest`); the
   `hostProfileSourceProvenanceSchema` `.strict()` sub-object; and the
   `.strict()` `hostProfileSchema`. Reuse `entityIdSchema`, `sha256DigestSchema`,
   and `isoDateTimeSchema` from `common.ts`.
...
3. Add `packages/schemas/tests/host-profile.test.ts`: a well-formed profile
   accepts; each enum value accepts and out-of-enum rejects; `host_identity.digest`
   rejects a raw-UUID / serial / non-`sha256:` shape (proving the raw identifier
   cannot land); `.strict()` rejects injected mint/value fields by name
   (`audit_chain_link_hash`, `producer`, `evidence_refs`, and a raw
   `platform_uuid` / `serial` / `machine_id` field); `source_provenance` wrong
   authority + non-strict reject; `os_version` rejects URI/path/secret shapes.
...
| Raw machine identifier at rest | Schema tests assert `host_identity.digest` rejects any non-`sha256:` shape (a raw UUID/serial cannot land) and `.strict()` rejects an injected raw `platform_uuid`/`serial` field; no synthetic trap at ADR acceptance. |
...
- `HostProfile` stays non-minted (no `audit_chain_link_hash` / producer /
  ...
  `sha256:` digest — no raw serial / UUID / `machine-id` field.
