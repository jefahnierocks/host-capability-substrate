...
exceptions. All 16 fields are kernel-set; producer-supplied Decision records
...
`audit_chain_link_hash`, `required_grant_kind`, and the ADR 0061 / D-059
additive nullable-optional attribution pair `policy_rule_ref` +
`resolved_policy_sha256` (kernel-set when populated; absent/null otherwise).
...
|---|---:|---|---|---|---|
| `AgentClient` | 2.1.1 | ADR 0037 | `packages/schemas/src/entities/agent-client.ts` | `packages/schemas/generated/AgentClient.schema.json` | Identity-axis enum mirrors and `remote_cloud_agent` surface extension recorded. ADR 0059 / D-058 later commits the canonical-hash discipline (non-version-bumping); AgentClient joins the mint/audit-service audit-chain commitment list — see §Current schema-version ledger and §Canonical-concatenation length-prefix discipline. |
...
|---|---:|---|---|
| `Evidence` | `0.11.0` | ADR 0076 / D-077 adds the `model` subject kind to `evidenceSubjectKindSchema` (a genuinely-new subject-kind value bumps the contract). 0.10.0 added the `evidenceAuthoritySchema` `self-asserted` enum extension (closes ADR 0039 §Forward-looking observations #5). | A model fact is observed via a `subject_kind: 'model'` Evidence record (`subject_id` = a `Model.model_id`; ADR 0076). Direct Evidence subtype payloads use the base envelope and their own `payload_schema_version` values; pre-reserved-value fulfillments (Q-014/Q-015 ADR 0044/0045, the storage primitives Artifact/Lock/ResourceBudget) did NOT bump this contract — adding a NEW subject-kind value does (the documented widening discipline). |
...
| `Run` | `0.1.0` | ADR 0053 / D-041 foundational-entity introduction; ADR 0076 / D-077 additive `invoker_model_ref` | `runSchemaVersionSchema = z.literal('0.1.0')`. Landed at commit `7fb7e05`. ADR 0076 / D-077 adds the additive nullable-optional `invoker_model_ref` attribution FK (no version bump, the `policy_rule_ref` change class); `Run` is minted, so `invoker_model_ref` is EXCLUDED from the `audit_chain_link_hash` canonical concatenation (like `Decision.model_ref`) and is producer-asserted. |
...
| `HostProfile` | `0.1.0` | ADR 0066 / D-064 introduction (next lower-coupling M1 entity; closes RunnerHostObservation.host_id, ADR 0032) | `hostProfileSchemaVersionSchema = z.literal('0.1.0')`. NON-MINTED — no `audit_chain_link_hash`, no producer-mint field, no `evidence_refs`; absent from the ADR 0057 mint scope. The canonical durable host record: `host_profile_id` + `host_state` (active/retired) + `os_name` / `os_version` (bounded version regex) / `arch` + `host_identity` (`{ kind, sha256 digest }`, digest always present) + a `.strict()` `source_provenance` (disjoint `host_profile_declaration` authority). Reuses `entityIdSchema` + `sha256DigestSchema` + `isoDateTimeSchema`. The host fingerprint lives only as a non-reversible `sha256:` digest (a raw IOPlatformUUID/serial cannot land in `host_identity.digest`); `host_profile_id`-opaque (entityIdSchema accepts a raw-UUID shape — recorded accept-and-trap), installed-runtime + non-sandbox observation (inv. 8), digest computation, FK existence, and supersession are Ring 1 obligations. Distinct from the transient `RunnerHostObservation`; closes its `host_id` with NO RunnerHostObservation shape change. |
| `ToolProvider` | `0.1.0` | ADR 0067 / D-065 introduction (next lower-coupling M1 entity; head of the tool-resolution chain ToolProvider → ToolInstallation → ResolvedTool) | `toolProviderSchemaVersionSchema = z.literal('0.1.0')`. NON-MINTED — no `audit_chain_link_hash`, no producer-mint field, no `evidence_refs`; absent from the ADR 0057 mint scope. A durable tool-source record: `tool_provider_id` + `manager_kind` (mise/homebrew/system/project_local/unknown) + `provider_state` (active/retired) + an OPTIONAL `root_path` (a NEW `toolProviderRootPathSchema` provider-ROOT primitive) + a `.strict()` `source_provenance` (disjoint `tool_provider_declaration` authority + `observed_at`; lighter than the registry-bound PolicyRule/Capability provenance — no source-blob digest, since constructed per-use). Reuses `entityIdSchema` + `isoDateTimeSchema`. The tool-source field is named `manager_kind`, NOT `provider_kind`, to avoid the shipped `PullRequestReceipt.payload.provider_kind` + charter-inv-16-reserved `Capability.provider_kind` field-name collisions, and a distinct AXIS from ADR 0034 `install_source_kind`. `root_path` is a deliberate SIBLING of `toolProvenanceCanonicalPathSchema` (accepts bare provider roots incl. `/usr/local`; genuinely forbids `..` via `(?!.*\.\.)`), NOT a reuse. `tool_provider_id`-opaque (entityIdSchema accepts a raw-shape id — recorded accept-and-trap, mirroring HostProfile.host_profile_id), provider observation (installed-runtime + non-sandbox, inv. 8), FK existence (`ToolProvenance.tool_or_provider_ref`, `ToolInstallation`), and supersession are Ring 1 obligations. |
| `ToolInstallation` | `0.1.0` | ADR 0068 / D-066 introduction (next entity in the tool-resolution chain; the MIDDLE of ToolProvider → ToolInstallation → ResolvedTool) | `toolInstallationSchemaVersionSchema = z.literal('0.1.0')`. NON-MINTED — no `audit_chain_link_hash`, no producer-mint field, no `evidence_refs`; absent from the ADR 0057 mint scope. A durable per-install record: `tool_installation_id` + a REQUIRED `tool_provider_id` FK to `ToolProvider` (ADR 0067) + `tool_name` + `version` (both pinned to the `hostProfileOsVersionSchema` charset) + a NEW `install_surface_kind` enum (host_path/manager_shim/app_bundled/devcontainer/cloud_image/setup_script/unknown) + an OPTIONAL `install_path` + `installation_state` (active/retired) + a `.strict()` `source_provenance` (disjoint `tool_installation_declaration` authority + `observed_at`). Reuses `entityIdSchema` + `isoDateTimeSchema` + `toolProvenanceCanonicalPathSchema` (the OPTIONAL `install_path`). `install_surface_kind` is the AUTHORITY-SURFACE axis, distinct from `ToolProvider.manager_kind` (source) and ADR 0034 `install_source_kind` (mechanism). `install_path` REUSES the ADR 0034 tool-FILE-path primitive — the correct grain (an install path IS a tool file path), the INVERSE of ToolProvider.root_path's bare-root case. Fulfills the pre-reserved `Evidence.subject_kind: 'tool_installation'` with NO `evidenceSubjectKindSchema` change and NO `Evidence.schema_version` bump. `tool_installation_id`-opaque (entityIdSchema accepts a raw-shape id — recorded accept-and-trap, mirroring ToolProvider/HostProfile), `tool_provider_id` FK existence, install observation (installed-runtime + non-sandbox, inv. 8), `install_path` deep canonicalization, and supersession are Ring 1 obligations. |
| `ResolvedTool` | `0.1.0` | ADR 0069 / D-067 introduction (the LAST entity in the tool-resolution chain; the TAIL of ToolProvider → ToolInstallation → ResolvedTool, completing it) | `resolvedToolSchemaVersionSchema = z.literal('0.1.0')`. NON-MINTED — no `audit_chain_link_hash`, no producer-mint field, no `evidence_refs`; absent from the ADR 0057 mint scope. The authoritative "what tool X resolves to in this context" answer: `resolved_tool_id` + `tool_name` (the query, pinned to the `hostProfileOsVersionSchema` charset) + a REQUIRED `tool_installation_id` FK to the winning `ToolInstallation` (ADR 0068) + a REQUIRED `execution_context_id` FK to `ExecutionContext` (ADR 0031; the resolving surface) + an OPTIONAL `workspace_id` FK to `WorkspaceContext` (ADR 0050; project scope) + a NEW `resolution_basis_kind` enum (path_order/workspace_pin/explicit_override/single_candidate/fallback/unknown) + `resolution_state` (active/retired) + a `.strict()` `source_provenance` (disjoint `resolved_tool_declaration` authority + `observed_at`). Reuses `entityIdSchema` + `isoDateTimeSchema`. `resolution_basis_kind` is the RESOLUTION-BASIS axis (WHY this install won), distinct from `ToolProvider.manager_kind` (source) / ADR 0034 `install_source_kind` (mechanism) / `ToolInstallation.install_surface_kind` (surface); its values are disjoint from `install_surface_kind`'s SUBSTANTIVE values, the two sharing ONLY the universal `unknown` house sentinel. Fulfills the pre-reserved `Evidence.subject_kind: 'resolved_tool'` with NO `evidenceSubjectKindSchema` change and NO `Evidence.schema_version` bump. `resolved_tool_id`-opaque (entityIdSchema accepts a raw-shape id — recorded accept-and-trap, mirroring ToolInstallation/ToolProvider/HostProfile), the three FKs' existence, computing the resolution (non-sandbox, inv. 8), basis↔context cross-consistency, and supersession are Ring 1 obligations. |
| `Artifact` | `0.1.0` | ADR 0070 / D-068 introduction (the first of the remaining lower-coupling "storage primitive" M1 entities — Artifact / Lock / ResourceBudget) | `artifactSchemaVersionSchema = z.literal('0.1.0')`. NON-MINTED — no `audit_chain_link_hash`, no producer-mint field, no `evidence_refs`; absent from the ADR 0057 mint scope. A DIGEST-ADDRESSED, IMMUTABLE descriptor of a run's output ("a run's structured output: diff / log chunks / exit code / signed summary"): `artifact_id` + a REQUIRED `run_id` FK to `Run` (ADR 0053) + an `artifact_kind` enum (command_diff/log_chunk/exit_code/signed_summary/unknown) + `content_sha256` (`sha256DigestSchema` — the content ADDRESS, NOT the bytes) + `byte_size` (`z.number().int().min(0)`) + a `.strict()` `source_provenance` (disjoint `artifact_declaration` authority + `observed_at`). Reuses `entityIdSchema` + `isoDateTimeSchema` + `sha256DigestSchema`. NO inline content, NO storage-location pointer, NO inline value, NO lifecycle-state field — Ring 0 holds no runtime blobs (they live in Ring 1 runtime state) and no secret-shaped values at rest (inv. 1/5); the sha256 digest IS the artifact identity. IMMUTABLE: a corrected/re-run output is a NEW Artifact with a NEW digest (so no active/retired state, also the policy-cleaner choice). Fulfills the pre-reserved `Evidence.subject_kind: 'artifact'` with NO `evidenceSubjectKindSchema` change and NO `Evidence.schema_version` bump. `artifact_id`-opaque (entityIdSchema accepts a raw-shape id — recorded accept-and-trap, mirroring the tool-chain peers), `run_id` FK existence, `content_sha256` digest verification + the `byte_size`↔digest cross-consistency, the canonical-encoding rule, sandbox non-promotion (inv. 8), and retention/GC are Ring 1 obligations. |
| `Lock` | `0.1.0` | ADR 0071 / D-069 introduction (the second of the remaining lower-coupling "storage primitive" M1 entities — Artifact / Lock / ResourceBudget) | `lockSchemaVersionSchema = z.literal('0.1.0')`. NON-MINTED — no `audit_chain_link_hash`, no producer-mint field, no `evidence_refs`, no per-resource scope, no `force_break_grant_id`; absent from the ADR 0057 mint scope (it is the COARSE, lighter, non-minted cousin of the MINTED `Lease`, ADR 0052, carrying none of Lease's minting/authorization machinery — so it cannot stand in for or bypass a Lease). A coarse mutex (e.g. package-manager global): `lock_id` + a CLOSED `lock_kind` enum (package_manager_global/host_mutation_global/unknown) + a REQUIRED `held_by_session_id` FK to `Session` (ADR 0055; reuses `Lease.held_by_session_id`, same semantic) + a `lock_status` enum (held/released) + a `.strict()` `source_provenance` (disjoint `lock_declaration` authority + `observed_at`). Reuses `entityIdSchema` + `isoDateTimeSchema`. The lifecycle field is `lock_status`, NOT `lock_state`, to avoid the shipped same-name `GitWorktreeObservation.payload.lock_state` (`gitWorktreeLockStateSchema` = unlocked/locked/held_by_other_session, a semantically different git-worktree posture). `lock_kind` is distinct from `Lease`'s per-resource scope (a coarse named mutex, not a specific resource). Fulfills the pre-reserved `Evidence.subject_kind: 'lock'` with NO `evidenceSubjectKindSchema` change and NO `Evidence.schema_version` bump. `lock_id`-opaque (entityIdSchema accepts a raw-shape id — recorded accept-and-trap), mutual-exclusion enforcement (one held lock per lock_kind), acquire/release + holder-only release, `held_by_session_id` FK existence, holder + `lock_status` sandbox non-promotion (inv. 8), and supersession are Ring 1 obligations. |
| `ResourceBudget` | `0.1.0` | ADR 0072 / D-070 introduction (the THIRD and FINAL of the lower-coupling "storage primitive" M1 entities — Artifact / Lock / ResourceBudget — closing the M1 22-entity Ring-0 set 22/22) | `resourceBudgetSchemaVersionSchema = z.literal('0.1.0')`. NON-MINTED — no `audit_chain_link_hash`, no producer-mint field, no `evidence_refs`; absent from the ADR 0057 mint scope. A per-session resource ALLOCATION (one record per `session` × `resource`): `resource_budget_id` + a REQUIRED `session_id` FK to `Session` (ADR 0055) + a `resource_kind` enum (cpu/memory/network/sandbox_concurrency/unknown) + `limit_value` (`z.number().int().min(0)`) + a `limit_unit` enum (cores/bytes/bytes_per_sec/count/percent/unknown) + a `budget_state` enum (active/retired) + a `.strict()` `source_provenance` (disjoint `resource_budget_declaration` authority + `observed_at`). Reuses `entityIdSchema` + `isoDateTimeSchema`. The durable ALLOCATION (a limit), distinct from the shipped `ResourceBudgetObservation` (the Evidence pressure reading that feeds it; distinct export names `resourceBudgetSchema` vs `resourceBudgetObservationSchema`). The lifecycle field is `budget_state` (the `_state` convention; renamed from the proposed `budget_status` for peer consistency with `resolution_state`/`installation_state`/`provider_state`). Fulfills the pre-reserved `Evidence.subject_kind: 'resource_budget'` with NO `evidenceSubjectKindSchema` change and NO `Evidence.schema_version` bump. `resource_budget_id`-opaque (entityIdSchema accepts a raw-shape id — recorded accept-and-trap, mirroring the storage-primitive peers), `session_id` FK existence, the `resource_kind`↔`limit_unit` consistency (NOT cross-constrained at Ring 0 — a recorded accept-and-trap, mirroring the ResolvedTool basis↔context non-cross-constraint), sandbox non-promotion (inv. 8), supersession, and enforcement against `ResourceBudgetObservation` pressure are Ring 1 obligations. NO policy-tier denylist (inv. 1). |
| `Model` | `0.1.0` | ADR 0076 / D-077 introduction (the FIRST post-M1 Ring-0 entity — the 23rd; the spec-driven model-as-object that COMPLETES ADR 0075) | `modelSchemaVersionSchema = z.literal('0.1.0')`. NON-MINTED — no `audit_chain_link_hash`, no producer-mint field, no `evidence_refs`; absent from the ADR 0057 mint scope. A first-class typed record of a served LLM/agent model: `model_id` + `vendor` enum (anthropic/openai/google/unknown) + `runtime_family` enum (claude/codex/gemini_adk/unknown) + an OPTIONAL `tier` token + `pin_form` enum (alias/exact_id/profile_handle) + `pin_value` token + an OPTIONAL `resolved_model_name` token + an OPTIONAL `context_window` (positive int) + a `model_state` enum (announced/active/deprecated/retracted/superseded) + an OPTIONAL `supersedes_model_id` self-FK + an OPTIONAL `decision_ledger_row` token (a `DECISIONS.md` row id, NOT a `Decision` FK — so deliberately NOT a `_ref`) + a `.strict()` `source_provenance` (a MULTI-VALUE `modelSourceAuthoritySchema` — vendor_published/observed_runtime/operator_attested — disjoint from `evidenceAuthoritySchema`, typing the inv-14 split; HCS's first non-minted entity with a multi-value declaration-site authority). The bounded tokens (`tier`/`pin_value`/`resolved_model_name`/`decision_ledger_row`) use the `/^[A-Za-z0-9._+-]+$/` charset (`.min(1).max(64)`, the `hostProfileOsVersion` family). Reuses `entityIdSchema` + `isoDateTimeSchema`. COMPLETES ADR 0075: it MIRRORS, never competes with, the AGENTS.md §Tool baseline + `DECISIONS.md` re-baseline-row authority of record (via the `decision_ledger_row` anchor). The model VERSION lives only on the `active` record (`resolved_model_name`/`tier`); the de-versioning rule (ADR 0075) becomes a typed invariant. Observed via the NEW `Evidence.subject_kind: 'model'` — which DOES bump `Evidence.schema_version` 0.10.0→0.11.0 (a genuinely-new subject kind, unlike the pre-reserved-value storage primitives). The additive nullable-optional `Decision.model_ref` + `Run.invoker_model_ref` attribution FKs land alongside (no `Decision`/`Run` schema_version bump; EXCLUDED from each minted entity's `audit_chain_link_hash` canonical concatenation). `model_id`/`supersedes_model_id`-opaque (entityIdSchema accepts a raw-shape id — recorded accept-and-trap), FK existence, sandbox non-promotion (inv. 8), the `pin_form: exact_id ⇒ decision_ledger_row` rule (accept-and-trap at Ring 0), the retracted-model guard + inv-12 floor-rejection, and the `pin_value`→`resolved_model_name` resolver are Ring 1 obligations. NO policy-tier denylist (`tier` is the model capability RUNG, vocabulary-disjoint from policy tiers — inv. 1). |
...
and to ADR 0037 `AgentClient` by ADR 0059 / D-058):
...
concatenation per the jointly-committed posture. ADR 0059 commits
`AgentClient`'s explicit canonical field order and the literal `GENESIS`
sentinel, names the shortest-form unsigned varint codec, and pins the fixed
seven-slot `evidenceRefSchema` evidence-ref encoding (`evidence_id`, `source`,
`observed_at`, `valid_until`, `authority`, `parser_version`, `confidence`;
optional or nullable slots are never omitted). `AgentClient` thereby joins the
mint/audit-service audit-chain commitment list (Decision, ApprovalGrant, Lease,
Run, Principal, Session) for seven-entity coverage; producer attribution stays
service-path authority (`kernel_agent_client_resolver`, not a record field).
...
### `HostProfile` enum mirrors (ADR 0066)

HostProfile (ADR 0066 / D-064) is non-minted and closes
`RunnerHostObservation.host_id` (ADR 0032). The
`RunnerHostObservation.host_id → HostProfile.host_profile_id` FK is intentional;
the `host` vs `host_profile` noun asymmetry follows the existing
`workspace_id → WorkspaceContext` `<entity>_id` precedent.

#### `HostProfile` enum mirror (ADR 0066)

```
host_state:          active | retired
os_name:             macos | linux | windows | unknown
arch:                arm64 | x86_64 | unknown
host_identity.kind:  platform_uuid_sha256 | install_id_sha256 | unknown
```

All `lower_snake_case` (NO Sub-rule 9 grandfather needed). `host_state: retired`
is a valid historical record, not a policy-denied state. `host_identity.digest`
is a non-reversible `sha256:` digest (a raw machine identifier cannot land there);
the entity embeds NO policy-tier denylist (inv. 1).

### `ToolProvider` enum mirrors (ADR 0067)

ToolProvider (ADR 0067 / D-065) is non-minted and is the head of the
tool-resolution chain `ToolProvider → ToolInstallation → ResolvedTool`.

#### `ToolProvider` enum mirror (ADR 0067)

```
manager_kind:    mise | homebrew | system | project_local | unknown
provider_state:  active | retired
```

All `lower_snake_case` (NO Sub-rule 9 grandfather needed). The tool-source field
is named `manager_kind`, NOT `provider_kind`, to avoid the two existing
`provider_kind` field names — the SHIPPED `PullRequestReceipt.payload.provider_kind`
(VCS-host axis) and the charter-inv-16-RESERVED `Capability.provider_kind`
(external-control-plane axis) — and is a DISTINCT AXIS from ADR 0034
`ToolProvenance.install_source_kind` (the install-MECHANISM grain; cf. the
same-value-set `MCPCanonicalAuthority.canonical_install_source_kind`, ADR 0036):
`manager_kind` carries provider-grain `system` / `project_local` and omits the
per-tool `npm` / `pip` / `uv` / `asdf` / `manual` mechanisms — so the
`mise` / `homebrew` overlap across the three install-source-flavored enums is
deliberate, not drift. `provider_state: retired` is a valid historical record,
not a policy-denied state. The entity embeds NO policy-tier denylist (inv. 1).

### `ToolInstallation` enum mirrors (ADR 0068)

ToolInstallation (ADR 0068 / D-066) is non-minted and is the MIDDLE of the
tool-resolution chain `ToolProvider → ToolInstallation → ResolvedTool`.

#### `ToolInstallation` enum mirror (ADR 0068)

```
install_surface_kind:  host_path | manager_shim | app_bundled | devcontainer | cloud_image | setup_script | unknown
installation_state:    active | retired
```

All `lower_snake_case` (NO Sub-rule 9 grandfather needed). `install_surface_kind`
is the AUTHORITY-SURFACE axis — a THIRD axis distinct from `ToolProvider.manager_kind`
(the SOURCE grain — which source manages a family of tools) and ADR 0034
`ToolProvenance.install_source_kind` (the install-MECHANISM grain — how a tool was
installed): it answers WHERE/HOW the install is exposed (`app_bundled` is not
host-PATH truth, `devcontainer`/`cloud_image` are separate surfaces). It is also a
distinct field NAME from the connection-`surface` family (`surface` / `surface_id` /
`owning_surface` / `authority_surface_kind`). `installation_state: retired` is a
valid historical record, not a policy-denied state. The entity embeds NO
policy-tier denylist (inv. 1).

### `ResolvedTool` enum mirrors (ADR 0069)

ResolvedTool (ADR 0069 / D-067) is non-minted and is the TAIL of the
tool-resolution chain `ToolProvider → ToolInstallation → ResolvedTool`.

#### `ResolvedTool` enum mirror (ADR 0069)

```
resolution_basis_kind:  path_order | workspace_pin | explicit_override | single_candidate | fallback | unknown
resolution_state:       active | retired
```

All `lower_snake_case` (NO Sub-rule 9 grandfather needed). `resolution_basis_kind`
is the RESOLUTION-BASIS axis (WHY an install won) — the FOURTH tool axis, distinct
from `ToolProvider.manager_kind` (source), ADR 0034
`ToolProvenance.install_source_kind` (mechanism), and
`ToolInstallation.install_surface_kind` (authority surface). Its values are
disjoint from `install_surface_kind`'s SUBSTANTIVE values; the two enums share
ONLY the universal `unknown` house sentinel (present on all four axis enums) — a
shared-sentinel convention, NOT an axis overlap, so the "value-disjoint" line is
not absolute and a disjointness test rejects surface-ONLY values (manager_shim /
app_bundled / host_path / …) but NOT `unknown`. `resolution_state: retired` is a
valid historical record, not a policy-denied state. The entity embeds NO
policy-tier denylist (inv. 1).

### `Artifact` enum mirrors (ADR 0070)

Artifact (ADR 0070 / D-068) is non-minted and is the first storage-primitive
entity — a digest-addressed, immutable descriptor of a run's output.

#### `Artifact` enum mirror (ADR 0070)

```
artifact_kind:  command_diff | log_chunk | exit_code | signed_summary | unknown
```

All `lower_snake_case` (NO Sub-rule 9 grandfather needed). `artifact_kind` is the
output-shape FACT (a `command_diff` / `log_chunk` / `exit_code` / `signed_summary`,
or the universal `unknown` house sentinel) that Ring 1 reads as input — never a
verdict (inv. 1). It widens via the registered §Procedure rule. `Artifact` has NO
lifecycle-state enum (it is content-addressed + immutable, so a re-run is a new
digest = a new record); `content_sha256` is `sha256DigestSchema` (the content
ADDRESS, never the bytes/value at rest, inv. 5). The entity embeds NO policy-tier
denylist (inv. 1).

### `Lock` enum mirrors (ADR 0071)

Lock (ADR 0071 / D-069) is non-minted and is the second storage primitive — a
coarse mutex, the lighter cousin of the minted `Lease`.

#### `Lock` enum mirror (ADR 0071)

```
lock_kind:    package_manager_global | host_mutation_global | unknown
lock_status:  held | released
```

All `lower_snake_case` (NO Sub-rule 9 grandfather needed). `lock_kind` is the
CLOSED coarse-mutex class (distinct from `Lease`'s per-resource scope — a coarse
named mutex, not a specific resource), widenable via the registered §Procedure
rule + the `unknown` house sentinel. The lifecycle field is `lock_status`, NOT
`lock_state` — `lock_state` is the shipped `GitWorktreeObservation.payload.lock_state`
(`gitWorktreeLockStateSchema` = unlocked/locked/held_by_other_session, a
semantically DIFFERENT git-worktree posture), so the same NAME with a different
MEANING is avoided. The `released` VALUE overlap with `Lease.lease_state` is
intentional and harmless (different entity/field, same meaning). `lock_status:
released` is a valid historical record, not a policy-denied state. The entity
embeds NO policy-tier denylist (inv. 1).

### `ResourceBudget` enum mirrors (ADR 0072)

ResourceBudget (ADR 0072 / D-070) is non-minted and is the third and FINAL
storage primitive — a per-session resource allocation (closing the M1 22-entity
Ring-0 set). It is the durable ALLOCATION, distinct from the shipped
`ResourceBudgetObservation` (the Evidence pressure reading that feeds it).

#### `ResourceBudget` enum mirror (ADR 0072)

```
resource_kind:  cpu | memory | network | sandbox_concurrency | unknown
limit_unit:     cores | bytes | bytes_per_sec | count | percent | unknown
budget_state:   active | retired
```

All `lower_snake_case` (NO Sub-rule 9 grandfather needed). `resource_kind` is the
budgeted-dimension FACT and `limit_unit` the unit FACT that Ring 1 reads as input —
neither a verdict (inv. 1); both widen via the registered §Procedure rule + the
`unknown` house sentinel. The `resource_kind` values are the ALLOCATION dimension,
NOT the `ResourceBudgetObservation` pressure-payload vocabulary
(cpu_pressure_pct / memory_pressure_pct / ...). Ring 0 does NOT cross-constrain
`resource_kind` ↔ `limit_unit` (a recorded accept-and-trap routed to Ring 1,
mirroring the `ResolvedTool` basis↔context non-cross-constraint). The lifecycle
field is `budget_state` (the `_state` convention, peer of `resolution_state` /
`installation_state` / `provider_state`); `retired` is a valid historical record,
not a policy-denied state. The entity embeds NO policy-tier denylist (inv. 1).

### `Model` enum mirrors (ADR 0076)

Model (ADR 0076 / D-077) is non-minted and is the first post-M1 Ring-0 entity — the
spec-driven model-as-object that completes ADR 0075 (which single-sourced the model
STRING in prose). It also adds the `model` `Evidence.subject_kind` (bumping
`Evidence.schema_version` 0.10.0→0.11.0) and the additive `Decision.model_ref` /
`Run.invoker_model_ref` attribution FKs.

#### `Model` enum mirror (ADR 0076)

```
vendor:          anthropic | openai | google | unknown
runtime_family:  claude | codex | gemini_adk | unknown
pin_form:        alias | exact_id | profile_handle
model_state:     announced | active | deprecated | retracted | superseded
source_provenance.authority (modelSourceAuthoritySchema):
                 vendor_published | observed_runtime | operator_attested
```

All `lower_snake_case` (NO Sub-rule 9 grandfather needed). `vendor` / `runtime_family` /
`tier` are descriptive model FACTS Ring 1 reads as input — none a verdict (inv. 1);
`vendor` / `runtime_family` / `model_state` widen via the registered §Procedure rule,
`vendor` / `runtime_family` carrying the `unknown` house sentinel (`pin_form` and
`model_state` are CLOSED — no sentinel: a pin is one of three forms, a lifecycle one of
five). `runtime_family` is the durable axis the de-versioning rule (ADR 0075) names
everywhere outside the single source; it includes `gemini_adk` (an eval-scope runtime)
even though Gemini is not yet an `AgentClient.product_family` / `ExecutionContext`
surface — the two enums are allowed to diverge by design (served-model family vs harness
product). `pin_form` types the Fable lesson (`alias` survives retraction; `exact_id`
fails closed and is the rationale-bearing inv-12 exception; `profile_handle` is the Codex
mechanism). `model_state` uses the `_state` lifecycle convention (peer of `host_state` /
`budget_state`), extended with `retracted` / `superseded`; the historical states are NOT
policy-denied (a retracted-model guard is Ring 1, inv. 1). `source_provenance.authority`
is HCS's FIRST multi-value non-minted declaration-site authority (the inv-14 split),
DISJOINT from `evidenceAuthoritySchema` and conferring no authority by itself. `tier` is
the model capability RUNG (opus/sonnet/haiku), a bounded token vocabulary-disjoint from
the policy tiers — the entity embeds NO policy-tier denylist (inv. 1).
...
- ADR 0066: `docs/host-capability-substrate/adr/0066-host-profile-ring-0-entity.md`
- ADR 0067: `docs/host-capability-substrate/adr/0067-tool-provider-ring-0-entity.md`
- ADR 0068: `docs/host-capability-substrate/adr/0068-tool-installation-ring-0-entity.md`
- ADR 0069: `docs/host-capability-substrate/adr/0069-resolved-tool-ring-0-entity.md`
- ADR 0070: `docs/host-capability-substrate/adr/0070-artifact-ring-0-entity.md`
- ADR 0071: `docs/host-capability-substrate/adr/0071-lock-ring-0-entity.md`
- ADR 0072: `docs/host-capability-substrate/adr/0072-resource-budget-ring-0-entity.md`
- ADR 0076: `docs/host-capability-substrate/adr/0076-model-ring-0-entity.md`
- Q-011: `DECISIONS.md`
...
| Version | Date | Change |
|---------|------|--------|
| 0.4.37 | 2026-06-23 | Recorded the ADR 0078 / D-081 `AgentClient` + `Session` model-attribution schema PR — adds `model_ref` to the two remaining minted entities ADR 0076 deferred, resolving the canonical-hash attribution-vs-identity fork **attribution-alongside**. Change-set: APPENDED a `model_ref`-exclusion note to the `AgentClient` and `Session` §Current-schema-version-ledger rows (both stay `0.1.0` — NO `schema_version` bump). `Session.model_ref` ("which model ran this session") and `AgentClient.model_ref` ("the model this client was configured to invoke by default at `kernel_observed_at`") are additive nullable-optional `entityIdSchema` FKs to `Model` (ADR 0076), **EXCLUDED** from each minted entity's `audit_chain_link_hash` canonical concatenation (attribution-field posture, like `Decision.model_ref` / `Run.invoker_model_ref`; identity-in-the-hash REJECTED — would reopen ADR 0059 + fragment client identity per model swap). The `AgentClient` `audit_chain_link_hash` `.describe()` gains one sentence recording `model_ref`'s exclusion (its exclusion enumeration stays complete; the `agentClientSchema.shape` canonical-order drift guard now filters `model_ref`); `Session` records the exclusion on the `model_ref` field describe (no inline canonical-order describe). Producer-asserted; FK existence, the `AgentClient` pin→`Model` resolution, and `subject_kind: 'model'` Evidence binding are Ring 1 obligations. Completes the four-locus model-attribution ladder (`Decision`/`Run`/`Session`/`AgentClient`). Regenerated `AgentClient.schema.json` + `Session.schema.json`; `agent-client.test.ts` gains a `model_ref` describe block + field-set-lock and drift-guard updates, `session.test.ts` gains a `model_ref` describe block (incl. the superRefine-independence case). Companion to ontology v1.34.0. |
| 0.4.36 | 2026-06-23 | Recorded the ADR 0076 / D-077 `Model` schema PR — the FIRST post-M1 Ring-0 entity (the 23rd): the spec-driven model-as-object that COMPLETES ADR 0075 (which single-sourced the model STRING in prose) by typing model identity/spec/lifecycle. Change-set: NEW §Current-schema-version-ledger row (`Model @ '0.1.0'`, non-minted); BUMPED the `Evidence` ledger row `0.10.0 → 0.11.0` (the genuinely-new `model` subject kind); NEW §Schema-enum-mirrors `Model` subsection (vendor / runtime_family / pin_form / model_state / modelSourceAuthority); NEW §References row for ADR 0076. `modelSchema` (.strict()): model_id + vendor enum (anthropic/openai/google/unknown) + runtime_family enum (claude/codex/gemini_adk/unknown) + OPTIONAL tier token + pin_form enum (alias/exact_id/profile_handle) + pin_value token + OPTIONAL resolved_model_name token + OPTIONAL context_window (positive int) + model_state enum (announced/active/deprecated/retracted/superseded) + OPTIONAL supersedes_model_id self-FK + OPTIONAL decision_ledger_row token (a DECISIONS.md row id, NOT a Decision FK — deliberately NOT a `_ref`) + .strict() source_provenance with a MULTI-VALUE modelSourceAuthoritySchema (vendor_published/observed_runtime/operator_attested — HCS's first non-minted entity with a multi-value declaration-site authority, the inv-14 split). Reuses entityIdSchema + isoDateTimeSchema; bounded tokens on the `/^[A-Za-z0-9._+-]+$/` charset (.min(1).max(64)). NON-MINTED — no audit_chain_link_hash/producer/evidence_refs; absent from the ADR 0057 mint scope. Observed via the NEW `Evidence.subject_kind: 'model'`. Cross-entity additive changes: `evidenceSubjectKindSchema += 'model'` (bumps Evidence.schema_version 0.10.0→0.11.0); `Decision.model_ref` + `Run.invoker_model_ref` (additive nullable-optional attribution FKs, NO Decision/Run schema_version bump, EXCLUDED from each minted entity's `audit_chain_link_hash` canonical concatenation — the AgentClient-excludes-producer posture; producer-asserted, Ring-1-bound). COMPLETES ADR 0075: MIRRORS, never competes with, the AGENTS.md §Tool baseline + DECISIONS.md re-baseline-row authority of record (the decision_ledger_row anchor); the de-versioning rule becomes a typed invariant. model_id/supersedes_model_id-opaque (entityIdSchema accepts a raw-shape id — recorded accept-and-trap), FK existence, sandbox non-promotion (inv. 8), the pin_form:exact_id⇒decision_ledger_row rule (accept-and-trap at Ring 0), the retracted-model guard + inv-12 floor-rejection, and the pin_value→resolved_model_name resolver are Ring 1 obligations. NO policy-tier denylist (tier is the model capability RUNG, vocabulary-disjoint from policy tiers — inv. 1). Regenerated Model.schema.json + Evidence/Decision/Run JSON Schema; added model.test.ts; bumped the base-Evidence schema_version fixtures across the existing test suite 0.10.0→0.11.0 (69 fixtures, the genuinely-new-subject-kind churn). Companion to ontology v1.33.0. |
...
| 0.4.31 | 2026-06-08 | Recorded the ADR 0070 / D-068 `Artifact` schema PR — the non-minted Ring-0 first storage-primitive entity: a DIGEST-ADDRESSED, IMMUTABLE descriptor of a run's output (diff / log chunk / exit code / signed summary). Change-set: NEW §Current-schema-version-ledger row (`Artifact @ '0.1.0'`, non-minted); NEW §Schema-enum-mirrors `Artifact` subsection (artifact_kind) recording the descriptor-not-bytes + immutable-no-state + Evidence-subject-kind-fulfilled posture WITH the `unknown` house-sentinel + §Procedure-widening note; NEW §References row for ADR 0070. `artifactSchema` (.strict()): artifact_id + a REQUIRED run_id FK to Run (ADR 0053) + an artifact_kind enum (command_diff/log_chunk/exit_code/signed_summary/unknown) + content_sha256 (sha256DigestSchema — the content ADDRESS, NOT the bytes) + byte_size (z.number().int().min(0)) + .strict() source_provenance (disjoint `artifact_declaration` authority). Reuses `entityIdSchema` + `isoDateTimeSchema` + `sha256DigestSchema`. NO inline content, NO storage-location pointer, NO inline value, NO lifecycle-state field — Ring 0 holds no runtime blobs (they live in Ring 1 runtime state) and no secret-shaped values at rest (inv. 1/5); the sha256 digest IS the identity. IMMUTABLE (a re-run = a new digest = a new Artifact). Fulfills the pre-reserved `Evidence.subject_kind: 'artifact'` with NO `evidenceSubjectKindSchema` change and NO `Evidence.schema_version` bump. artifact_id-opaque (entityIdSchema accepts a raw-shape id — recorded accept-and-trap, mirroring the tool-chain peers), run_id FK existence, content_sha256 digest verification + byte_size↔digest cross-consistency, the canonical-encoding rule, sandbox non-promotion (inv. 8), and retention/GC are Ring 1 obligations. NO policy-tier denylist (inv. 1). Regenerated Artifact.schema.json; added artifact.test.ts. |
| 0.4.30 | 2026-06-08 | Recorded the ADR 0069 / D-067 `ResolvedTool` schema PR — the non-minted Ring-0 authoritative resolution answer and TAIL of the tool-resolution chain `ToolProvider → ToolInstallation → ResolvedTool` (completing it at source). Change-set: NEW §Current-schema-version-ledger row (`ResolvedTool @ '0.1.0'`, non-minted); NEW §Schema-enum-mirrors `ResolvedTool` subsection (resolution_basis_kind / resolution_state) documenting the FOUR-axis disambiguation (resolution_basis_kind WHY-it-won vs manager_kind source vs install_source_kind mechanism vs install_surface_kind surface) WITH the `unknown`-is-shared-house-sentinel carve-out (basis values disjoint from install_surface_kind's SUBSTANTIVE values; the disjointness test rejects surface-ONLY values, NOT `unknown`); NEW §References row for ADR 0069. `resolvedToolSchema` (.strict()): resolved_tool_id + tool_name (the query, pinned to the `hostProfileOsVersionSchema` charset) + a REQUIRED tool_installation_id FK (winning install, ADR 0068) + a REQUIRED execution_context_id FK (ExecutionContext, ADR 0031) + an OPTIONAL workspace_id FK (WorkspaceContext, ADR 0050) + a NEW resolution_basis_kind enum (path_order/workspace_pin/explicit_override/single_candidate/fallback/unknown) + resolution_state (active/retired) + .strict() source_provenance (disjoint `resolved_tool_declaration` authority). Reuses `entityIdSchema` + `isoDateTimeSchema`. Fulfills the pre-reserved `Evidence.subject_kind: 'resolved_tool'` with NO `evidenceSubjectKindSchema` change and NO `Evidence.schema_version` bump. resolved_tool_id-opaque (entityIdSchema accepts a raw-shape id — recorded accept-and-trap, mirroring ToolInstallation/ToolProvider/HostProfile), the three FKs' existence, computing the resolution (non-sandbox, inv. 8), basis↔context cross-consistency, and supersession are Ring 1 obligations. NO policy-tier denylist (inv. 1). Regenerated ResolvedTool.schema.json; added resolved-tool.test.ts. |
| 0.4.29 | 2026-06-08 | Recorded the ADR 0068 / D-066 `ToolInstallation` schema PR — the non-minted Ring-0 durable per-install record and MIDDLE of the tool-resolution chain `ToolProvider → ToolInstallation → ResolvedTool`. Change-set: NEW §Current-schema-version-ledger row (`ToolInstallation @ '0.1.0'`, non-minted); NEW §Schema-enum-mirrors `ToolInstallation` subsection (install_surface_kind / installation_state) documenting the three-axis disambiguation (install_surface_kind authority-surface vs `ToolProvider.manager_kind` source vs ADR 0034 `install_source_kind` mechanism) and the distinct field NAME from the connection-`surface` family; NEW §References row for ADR 0068. `toolInstallationSchema` (.strict()): tool_installation_id + a REQUIRED tool_provider_id FK to ToolProvider + tool_name + version (both pinned to the `hostProfileOsVersionSchema` charset) + a NEW install_surface_kind enum (host_path/manager_shim/app_bundled/devcontainer/cloud_image/setup_script/unknown) + an OPTIONAL install_path + installation_state (active/retired) + .strict() source_provenance (disjoint `tool_installation_declaration` authority). install_path REUSES the ADR 0034 tool-FILE-path `toolProvenanceCanonicalPathSchema` — the correct grain (an install path IS a tool file path), the INVERSE of ToolProvider.root_path's bare-root case (which needed a sibling). Fulfills the pre-reserved `Evidence.subject_kind: 'tool_installation'` with NO `evidenceSubjectKindSchema` change and NO `Evidence.schema_version` bump. tool_installation_id-opaque (entityIdSchema accepts a raw-shape id — recorded accept-and-trap, mirroring ToolProvider/HostProfile), tool_provider_id FK existence, install observation (installed-runtime + non-sandbox, inv. 8), install_path deep canonicalization, and supersession are Ring 1 obligations. NO policy-tier denylist (inv. 1). Regenerated ToolInstallation.schema.json; added tool-installation.test.ts. |
| 0.4.28 | 2026-06-08 | Recorded the ADR 0067 / D-065 `ToolProvider` schema PR — the non-minted Ring-0 durable tool-source entity and head of the tool-resolution chain `ToolProvider → ToolInstallation → ResolvedTool`. Change-set: NEW §Current-schema-version-ledger row (`ToolProvider @ '0.1.0'`, non-minted); NEW §Schema-enum-mirrors `ToolProvider` subsection (manager_kind / provider_state) cross-referencing all three `provider_kind`-family/install-source field names (manager_kind vs the shipped `PullRequestReceipt.provider_kind` vs the charter-inv-16-reserved `Capability.provider_kind`; distinct axis from ADR 0034 `install_source_kind` + the same-value ADR 0036 `canonical_install_source_kind`); NEW §References row for ADR 0067. `toolProviderSchema` (.strict()): tool_provider_id + manager_kind + provider_state + optional root_path + .strict() source_provenance (disjoint `tool_provider_declaration` authority). The tool-source field is named manager_kind (NOT provider_kind) to avoid the two shipped/reserved provider_kind field-name collisions and stay a distinct axis from install_source_kind. root_path uses a NEW `toolProviderRootPathSchema` provider-ROOT sibling (accepts bare provider roots incl. /usr/local; genuinely forbids `..` via `(?!.*\.\.)`) — NOT a reuse of the tool-file-path `toolProvenanceCanonicalPathSchema` (which rejects bare /usr/local and does not structurally reject `..`). tool_provider_id-opaque (entityIdSchema accepts a raw-shape id — recorded accept-and-trap, mirroring HostProfile.host_profile_id), provider observation (installed-runtime + non-sandbox, inv. 8), FK existence, and supersession are Ring 1 obligations. NO policy-tier denylist (inv. 1). Regenerated ToolProvider.schema.json; added tool-provider.test.ts. |
| 0.4.27 | 2026-06-07 | Recorded the ADR 0066 / D-064 `HostProfile` schema PR — the non-minted Ring-0 canonical host identity + stable facts entity closing RunnerHostObservation.host_id (ADR 0032). Change-set: NEW §Current-schema-version-ledger row (`HostProfile @ '0.1.0'`, non-minted); NEW §Schema-enum-mirrors `HostProfile` subsection (host_state / os_name / arch / host_identity.kind) recording the `RunnerHostObservation.host_id → HostProfile.host_profile_id` FK mapping (host vs host_profile noun asymmetry per the workspace_id → WorkspaceContext precedent); NEW §References row for ADR 0066. `hostProfileSchema` (.strict()): host_profile_id + host_state + os_name/os_version (bounded version regex)/arch + host_identity ({kind, sha256 digest}) + .strict() source_provenance (disjoint host_profile_declaration authority). The never-the-raw-identifier guarantee is structural for host_identity.digest (a raw IOPlatformUUID/serial cannot match ^sha256:[a-f0-9]{64}$) but NOT host_profile_id (entityIdSchema accepts a raw-UUID shape — recorded accept-and-trap; host_profile_id-opaque is a Ring 1 obligation). NO policy-tier denylist (inv. 1); host facts are read-only policy inputs. No RunnerHostObservation shape change (host_id stays entityIdSchema.optional(); describe()-only FK-target note). Regenerated HostProfile.schema.json + RunnerHostObservation.schema.json (describe-only); added host-profile.test.ts. |
