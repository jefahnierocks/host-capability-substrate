...
`Session.principal_id` (landed, ADR 0055). Also closes ADR 0025 §Branch deletion
...
- `env` (`z.array(commandShapeEnvEntrySchema)`, may be empty) carries variable
  **names** plus a typed value-**source** reference, never resolved values
  ...
  on `kind`: `secret_reference` (an opaque `secret_reference_ref` forward FK to
  the `SecretReference` entity (built by ADR 0065 / D-063); the broker resolves
  the value at execution time) or `execution_context_inherited` (no value
  stored). No variant
...
Source: `packages/schemas/src/entities/secret-reference.ts`

Typed secret reference (ADR 0065 / D-063), closing `CommandShape`'s forward
`secret_reference_ref` FK (ADR 0063). A NON-MINTED Ring-0 entity and structural
peer of `PolicyRule` / `Capability` / `CommandShape`: no `audit_chain_link_hash`,
no producer-mint field, no `evidence_refs`; absent from the ADR 0057 mint scope.

Key fields:

- `reference_kind` is a closed `lower_snake_case` enum: `op_uri`, `hcs_uri`,
  `keychain_item`, `env_var_name`, `broker_handle`.
- `reference_locator` is the opaque secret REFERENCE, validated per
  `reference_kind` by the envelope `superRefine`, and NEVER a resolved value
  (charter inv. 5). The STRUCTURED kinds (`op_uri` = `op://vault/item[/field]`,
  `keychain_item` = `keychain://service/account`) reject a mis-shaped or
  raw-value locator; the PERMISSIVE kinds (`env_var_name` name grammar,
  `hcs_uri` / `broker_handle` opaque tails) accept a token-shaped locator that
  satisfies the grammar — a recorded accept-and-trap gap (mirroring ADR 0063's
  argv-secret-inlining), backstopped by `forbidden-string-scan` and the deferred
  Ring 1 deep check. The schema embeds NO policy-tier denylist (charter inv. 1).
- `credential_source_ref` is an OPTIONAL FK to a durable `CredentialSource`
  (ADR 0018); the `_ref` suffix follows the optional/forward cross-record-FK
  precedent, distinct from the kernel-set
  `BoundaryObservation.credential_source_id`.
- `source_provenance` is a `.strict()` declaration-site binding (disjoint
  `authority` literal `secret_reference_declaration` + `observed_at`), lighter
  than the registry-bound `PolicyRule` / `Capability` provenance because a
  `SecretReference` is constructed per-use, not read from a hash-bound blob.

`SecretReference` is the **secret-reference** slot, disjoint from
`ProviderObjectReference` / `PublicClientId` / `PolicySelectorValue` / raw secret
material (charter inv. 16 / line 98). Value resolution, FK existence, the deep
...
`ToolInstallation`), and supersession — are Ring 1 obligations. A tool
source is a read-only policy INPUT to Ring 1, never policy content (inv. 1).

### `ToolInstallation`

Source: `packages/schemas/src/entities/tool-installation.ts`

Durable per-install record (ADR 0068 / D-066) — a specific instance of a tool on
this host (e.g. `node` `24.3.0` installed by a `mise` provider). The MIDDLE of
the tool-resolution chain `ToolProvider → ToolInstallation → ResolvedTool`. A
NON-MINTED Ring-0 entity and structural peer of `ToolProvider` / `HostProfile` /
`Capability` / `SecretReference`: no `audit_chain_link_hash`, no producer-mint
field, no `evidence_refs`; absent from the ADR 0057 mint scope.

Key fields:

- `tool_provider_id` is a REQUIRED FK to the `ToolProvider` (ADR 0067) this
  install comes from; a `system` / `manual` install is recorded against a
  provider whose `manager_kind` is `system` / `unknown`. FK existence is a Ring 1
  obligation.
- `tool_name` and `version` are bounded fact strings (pinned to the
  `hostProfileOsVersionSchema` charset — no whitespace / `/` / `:` / path / secret
  shape; bounded length), e.g. `node` `24.3.0`.
- `install_surface_kind` is the NEW authority-surface enum: `host_path` |
  `manager_shim` | `app_bundled` | `devcontainer` | `cloud_image` |
  `setup_script` | `unknown`. It is a THIRD axis, distinct from
  `ToolProvider.manager_kind` (the SOURCE grain) and ADR 0034
  `ToolProvenance.install_source_kind` (the install-MECHANISM grain): `app_bundled`
  is not host-PATH truth, etc. A descriptive FACT Ring 1 reads as input, never a
  trust verdict the entity carries (inv. 1).
- `install_path` is OPTIONAL and REUSES the ADR 0034
  `toolProvenanceCanonicalPathSchema` — an install path IS a tool FILE under a
  whitelisted root (the correct grain, the inverse of `ToolProvider.root_path`).
  Optional because `app_bundled` / `cloud_image` surfaces may have no canonical
  host file path. It inherits `ToolProvenance`'s canonicalization posture (URI /
  non-rooted rejected; deep `..` resolution a Ring 1 obligation).
- `installation_state` is `active` | `retired` (supersession via a NEW `active`
  record; `retired` is a historical record, not policy-denied).
- `source_provenance` is a `.strict()` declaration-site binding (disjoint
  `tool_installation_declaration` authority + `observed_at`).
...
`Evidence.schema_version` bump. `tool_installation_id` is `entityIdSchema` and
accepts a raw-shape id, so id-opacity — plus install observation (installed-runtime,
...
Source: `packages/schemas/src/entities/artifact.ts`

A run's structured output (ADR 0070 / D-068): a diff, log chunk, exit code, or
signed summary. The first of the lower-coupling "storage primitive" entities. A
NON-MINTED Ring-0 entity and structural peer of `ToolInstallation` / `ResolvedTool`
/ `HostProfile` — MINUS their `active`/`retired` lifecycle: `Artifact` is IMMUTABLE
(content-addressed), so a corrected/re-run output is a NEW `Artifact` with a NEW
digest. No `audit_chain_link_hash`, no producer-mint field, no `evidence_refs`;
absent from the ADR 0057 mint scope.

Key fields:

- `run_id` is a REQUIRED FK to the `Run` (ADR 0053) that produced the output.
- `artifact_kind` is `command_diff` | `log_chunk` | `exit_code` | `signed_summary`
  | `unknown` — the output shape; a descriptive FACT, never a verdict (inv. 1).
- `content_sha256` is the content ADDRESS (`sha256DigestSchema`): a non-reversible
  digest of the output's canonical bytes. NEVER the bytes, NEVER a secret value
  (inv. 5). The digest IS the artifact's identity; the bytes and their storage
  location are Ring 1 runtime state (charter: no runtime blobs in Ring 0 / the repo).
- `byte_size` is a non-negative integer size FACT.
- `source_provenance` is a `.strict()` declaration-site binding (disjoint
  `artifact_declaration` authority + `observed_at`).

`Artifact` carries NO inline content, NO storage-location pointer, NO inline value,
and NO lifecycle-state field. It fulfills the pre-reserved `Evidence.subject_kind:
'artifact'` with NO `evidenceSubjectKindSchema` change and NO `Evidence.schema_version`
bump. `artifact_id` is `entityIdSchema` and accepts a raw-shape id, so id-opacity —
plus `run_id` FK existence, `content_sha256` digest verification + the
